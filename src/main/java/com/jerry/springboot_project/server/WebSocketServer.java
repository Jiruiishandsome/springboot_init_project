package com.jerry.springboot_project.server;
...
public class WebSocketServer {
    ...
    public static void sendInfo(Message message) {
        System.out.println(message.getSenderId()+"给" + message.getReceiveId() + "发送消息：" + message);
        if (webSocketMap.containsKey(message.getReceiveId().toString())) {
            try {
                webSocketMap.get(message.getReceiveId().toString()).session.getBasicRemote().sendText(JSON.toJSONString(message));
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
